...
(
	function ( global, factory )
	{
		...
		if ( typeof define === 'function' && define.amd ) {
			define (
				[ 'backbone', 'underscore', 'jquery' ], function ( backbone, underscore, jquery )
				{
	...
	this, function ( global, BackboneMVC, Backbone, _, $ )
	{
		...
		var ControllerSingleton = (
			function ()
			{
				function BaseController ()
				{
					this._created = (
						new Date ()
					).getTime (); //this can be useful for development
				}
		...
		function _extendMethodGenerator ( klass, _inheritedMethodsDefinition )
		{
			...
			return function ( properties )
			{
				...
				_.each (
					properties, function ( value, propertyName )
					{
				...
				_.extend (
					tmpControllerProperties, actionMethods, {
		...
		function camelCased ( string )
		{
			...
			return (
				_.map (
					string.split ( ' ' ), function ( entry )
					{
						return entry.replace (
							/(^|_)[a-z]/gi, function ( match )
							{
