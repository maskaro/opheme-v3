...
(
	function ( factory )
	{
		...
		if ( typeof require === "function" && typeof exports === "object" && typeof module === "object" ) {
			...
			define (
				[ "jquery", "exports" ], function ( $, exports )
				{
					factory ( window, Math, $, exports );
				}
			);
		}
		else {
		...
		function ( window, Math, $, exports, oldInfinity )
		{
			...
			{
				var firstPage, pages = listView.pages,
			...
			ListView.prototype.append = function ( obj )
			{
				...
				var lastPage, item = convertToItem ( this, obj ),
				    pages          = this.pages;
			...
			ListView.prototype.prepend = function ( obj )
			{
				...
				var firstPage, item = convertToItem ( this, obj, true ),
				    pages           = this.pages;
			...
			{
				var index, length, curr, pages = listView.pages,
				    inserted                   = false,
				    inOrder                    = true;
			...
			{
				var index, length, pages, lastIndex, nextLastIndex, startIndex = listView.startIndex,
				...
				if ( nextIndex < 0 || (
						nextIndex === startIndex && !prepended
					) ) {
					return startIndex;
				}
			...
			{
				var currPage, newPage, index, length, itemIndex, pageLength, currItems, currItem, nextItem, pages = listView.pages,
				    newPages                                                                                      = [];
			...
			ListView.prototype.repartition = repartition;
			...
			ListView.prototype.find = function ( findObj )
			{
				...
				findObj.each (
					function ()
					{
						var pageId, page, pageItems, index, length, currItem, $itemEl = $ ( this ).parentsUntil ( '[' + PAGE_ID_ATTRIBUTE + ']' ).andSelf ().first (),
			...
			{
				var index, length, curr, startIndex, midpoint, diff, prevDiff, pages = listView.pages,
				    rangeMidpoint                                                    = top + (
				                                                                             bottom - top
				                                                                             ) / 2;
