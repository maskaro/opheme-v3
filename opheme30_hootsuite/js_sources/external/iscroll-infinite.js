	...
	function ( window, document, Math )
	{
			...
			function ()
			{
				...
				me.momentum = function ( current, start, time, lowerMargin, wrapperSize, deceleration )
				{
					...
					if ( destination < lowerMargin ) {
						destination = wrapperSize ? lowerMargin - (
						wrapperSize / 2.5 * (
						speed / 8
						)
						) : lowerMargin;
						...
						destination = wrapperSize ? wrapperSize / 2.5 * (
						speed / 8
						) : 0;
				...
				me.click = function ( e )
				{
					...
					if ( !(
							/(SELECT|INPUT|TEXTAREA)/i
						).test ( target.tagName ) ) {
						ev = document.createEvent ( 'MouseEvents' );
						ev.initMouseEvent (
							'click', true, true, e.view, 1,
		...
		IScroll.prototype = {
			...
			scrollTo: function ( x, y, time, easing )
			{
				...
				if ( !time || (
					this.options.useTransition && easing.style
					) ) {
				...
				this.on (
					'refresh', function ()
					{
						...
						this.infiniteUpperBufferSize = Math.floor (
							(
							this.infiniteLength - elementsPerPage
							) / 2
						);
