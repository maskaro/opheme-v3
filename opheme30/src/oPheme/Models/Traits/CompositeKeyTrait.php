...
{	
	/**
	 * 
	 * Overriding the Eloquent save method to allow for composite primary keys
	 * 
	 */
	...
	protected function performDeleteOnModel()
	{
		...
		// see if there is more than one primary key
		if( count($this->getKeyName()) > 1 ) {
			// modified code for composite primary keys
			$this->newQuery()->where( function($q) {
				foreach([$this->getKeyName()] as $key ) {
					// Fetch the primary key(s) values
					$unique = array_intersect_key($this->original, array_flip($key));
					// Apply SQL logic
					$q->where($unique);
				}
			})->delete();
